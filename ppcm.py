...
# La solution de Bézout n'est pas unique, on s'assure que a*x0 + b*y0 = d
if a * x0 + b * y0 != d:
x0 = -x0
y0 = -y0
...
# On évite la multiplication si possible pour prévenir l'overflow pour les très grands nombres
# bien que peu probable avec les entiers Python standard.
return abs(a * (b // d))
...
p = 2
# La condition p*p <= n est plus rapide que p <= math.sqrt(n)
while (p * p) <= n:
if is_prime[p]:
...
# =================================================================
def afficher_en_tableau(liste, colonnes=10, largeur_colonne=4):
"""
Affiche une liste de nombres sous forme de tableau (grille).
"""
print("-" * (colonnes * (largeur_colonne + 2) + 1))

# Créer le format de cellule, ex: "| 4 |"
format_cellule = "| {:>" + str(largeur_colonne) + "} "

for i in range(0, len(liste), colonnes):
ligne = liste[i:i + colonnes]
ligne_texte = "".join(format_cellule.format(n) for n in ligne)
# Compléter avec des cellules vides si la dernière ligne n'est pas pleine
lignes_vides = colonnes - len(ligne)
ligne_texte += "".join(format_cellule.format(" ") for _ in range(lignes_vides))
...
# Fonction pour formater le terme en 'k'
def format_k_term(coefficient):
if coefficient == 0:
return ""
...
terme = "k"
return f"{signe}{terme}"

print(f" x = {xp}{format_k_term(alpha)}")
print(f" y = {yp}{format_k_term(beta)}")
...
try:
n_max = int(input("Entrez le nombre maximum (N) pour trouver les nombres premiers (ex: 50) : "))
except ValueError:
...
# Affichage des nombres premiers dans un tableau
afficher_en_tableau(nombres_premiers)

print(f"\nTotal de **{len(nombres_premiers)}** nombres premiers trouvés.")
